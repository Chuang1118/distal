...
```{r depends-paths}
filt_path <- here::here("data/processed/02-sc_seurat_subset.rds")
```
...
```{r save}
saveRDS(subset, here::here("data/processed/06-sc_seurat_subset.rds"))
write.table(df_scores, here::here("output", DOCNAME,"06-raw_data.csv"), row.names=TRUE,col.names=TRUE, sep =',')
```
...
knitr::kable(data.frame(
     File = c(
         getDownloadLink2("06-raw_data.csv", str_sub(here::here("output", DOCNAME),2,-1)),
         getDownloadLink2("suppfigure4-panelA.png", str_sub(here::here("output", DOCNAME),2,-1)), # bug one "/" plus
         getDownloadLink2("suppfigure4-panelA.pdf", str_sub(here::here("output", DOCNAME),2,-1))
     ),
     Description = c(
         "raw data for source_data (CSV)",
         "Supplementary Figure 4 panel A (PNG)",
         "Supplementary Figure 4 panel A (PDF)"
     )
 ))
```
